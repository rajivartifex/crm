@extends('layouts.app')
@section('title')
    {{ $custCategory ? 'Edit Business Category' : 'Add Business Category' }}
@endsection
@section('style')
    <!-- Select2 -->
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12">
                    <h4>{{ $customer->cust_business_name ?? '' }} | Business Category | {{ $custCategory ? 'Edit' : 'Add' }}
                    </h4>
                </div>
                <div class="col-sm-6">
        ...
        <form class="business-category-form">
            ...
            <div class="card-body">
                <div class="row">
                            ...
                            <select class="form-control select2" style="width: 100%;" name="ff[categories_id]">
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id ?? '' }}"
                                        {{ $custCategory ? ($custCategory->categories_id == $category->id ? 'selected' : '') : '' }}>
                        ...
                        @foreach ($service_tags as $key => $tag)
                            <div class="col-sm-12">
                                <div class="row service_tag_wrapper">
                                    <input type="hidden" name="ff[service_tag_id]" class="service_tag_id"
                                        value="{{ $tag->id }}" />
                                    <div class="col-sm-11">
                                        <div class="form-group">
                                    ...
                                    <div class="col-sm-1">
                                        <div class="form-group">
                                            ...
                                            <button type="button" class="btn btn-sm btn-danger"
                                                data-id="{{ $tag->id }}" onclick="$(this).specialDelete();"><i
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            <a class="btn-sm btn btn-secondary add_service_tag_btn float-right"><i class="fas fa-plus"></i>
                                Service Tag</a>
                        </div>
                    </div>
                </div>
