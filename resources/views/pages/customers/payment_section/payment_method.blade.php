@extends('layouts.app')
@section('title')
    {{ $custPayment ? 'Edit Payment Methods Accepted' : 'Add Payment Methods Accepted' }}
@endsection
@section('style')
    <!-- Select2 -->
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12">
                    <h4>{{ $customer->cust_business_name ?? '' }} | Payment Methods Accepted |
                        {{ $custPayment ? 'Edit' : 'Add' }}</h4>
                </div>
                <div class="col-sm-6">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <select class="select2" multiple="multiple" name="ff[cust_payment_id][]"
                                data-placeholder="Select a Payment" style="width: 100%;">
                                @foreach ($payments as $pay)
                                    <option value="{{ $pay->id ?? '' }}"
                                        {{ $custPayment ? (in_array($pay->id, $custPayment->cust_payment_id) ? 'selected' : '') : '' }}>
                                        {{ $pay->payment ?? '' }}</option>
                                @endforeach
                            </select>
                        </div>
